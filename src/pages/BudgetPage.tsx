      ...
      <div className="mx-auto max-w-5xl px-4 py-3 md:px-8 md:py-5 space-y-6">
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center rounded-2xl border-2 border-dashed border-blue-100 py-20 text-center">
            ...
            <p className="mt-1 text-xs text-slate-400">Import your Excel file to get started.</p>
          </div>
        ) : (
          <>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              <div className="rounded-2xl border border-blue-100 bg-white p-5 shadow-sm col-span-2 md:col-span-1">
                ...
                <p className="mt-1 text-2xl font-bold text-slate-900">{usd(totalSpent)}</p>
              ...
              {!activeCat ? (
                <div className="rounded-2xl border border-blue-100 bg-white p-5 shadow-sm">
                  ...
                  {catBreakdown[0] ? (
                    <>
                      <p className="mt-1 text-lg font-bold text-slate-900">{catBreakdown[0].name}</p>
                      <p className="mt-0.5 text-xs text-slate-400">{usd(catBreakdown[0].amount)} · {Math.round(catBreakdown[0].amount / totalSpent * 100)}% of total</p>
                    </>
                ...
                <div className="rounded-2xl border border-blue-100 bg-white p-5 shadow-sm">
                  ...
                  {subcatBreakdown[0] ? (
                    <>
                      <p className="mt-1 text-lg font-bold text-slate-900">{subcatBreakdown[0].name}</p>
                      <p className="mt-0.5 text-xs text-slate-400">{usd(subcatBreakdown[0].amount)} · {Math.round(subcatBreakdown[0].amount / totalSpent * 100)}%</p>
                    </>
                  ) : <p className="mt-1 text-sm text-slate-400">—</p>}
                </div>
              )}
              <div className="rounded-2xl border border-blue-100 bg-white p-5 shadow-sm">
                ...
                {topTxns[0] ? (
                  <>
                    <p className="mt-1 text-lg font-bold text-slate-900">{usd(Math.abs(topTxns[0].amount))}</p>
                    <p className="mt-0.5 text-xs text-slate-400 truncate">{topTxns[0].description}</p>
                  </>
                ) : <p className="mt-1 text-sm text-slate-400">—</p>}
              </div>
            </div>
              ...
              <div className="md:col-span-2 rounded-2xl border border-blue-100 bg-white p-5 shadow-sm">
                ...
                {(() => {
                  ...
                  if (pieData.length === 0) return (
                    <div className="flex h-48 items-center justify-center text-xs text-slate-400">No data for this month</div>
                  )
                  return (
                    <ResponsiveContainer width="100%" height={200}>
                      <PieChart>
            ...
            <div className="rounded-2xl border border-blue-100 bg-white p-5 shadow-sm">
              ...
              {(() => {
                ...
                if (rows.length === 0) return <p className="text-sm text-slate-400 italic">No transactions this month.</p>
                return (
                          ...
                          <div className="flex items-center justify-between mb-1">
                            <div className="flex items-center gap-2">
                              ...
                              <button
                                ...
                                className={`text-sm text-slate-700 ${!activeCat ? 'hover:text-slate-900 hover:underline cursor-pointer' : ''}`}>
                                {row.name}
                              </button>
                            </div>
                            <div className="flex items-center gap-3">
                              ...
                              <span className="text-sm font-semibold text-slate-900 w-16 text-right tabular-nums">
                                {usd(row.amount)}
                              </span>
                            </div>
                          </div>
                  ...
                  {topTxns.map(t => (
                    <div key={t.id} className="flex items-center gap-3 py-2.5">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-slate-400">{format(parseISO(t.date), 'MMM d')} · {t.category}{t.subcategory ? ` · ${t.subcategory}` : ''}</p>
                      </div>
